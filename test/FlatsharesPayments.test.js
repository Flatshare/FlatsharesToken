...
contract('FlatsharesPayments Dispatch Test', function (accounts) {
    ...
    beforeEach(async function () {
        payments = await FlatsharesPayments.new(user1, aFee, { from: owner});
        token = await FlatsharesToken.new(user1, { from: owner});
    });
...
contract('FlatsharesPayments Cancel Test', function (accounts) {
    const owner = accounts[0];
    const user1 = accounts[1];
    const demand = accounts[2];
    const supply = accounts[3];
...
contract('FlatsharesPayments Arbitration Host Test', function (accounts) {
    const owner = accounts[0];
    const user1 = accounts[1];
    const demand = accounts[2];
    const supply = accounts[3];
    ...
    beforeEach(async function () {
        payments = await FlatsharesPayments.new(arbiterAddress, aFee, { from: owner});
        token = await FlatsharesToken.new(user1, { from: owner});
    });
