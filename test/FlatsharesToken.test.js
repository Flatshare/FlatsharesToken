...
contract('FlatsharesToken (Basic Tests)', function (accounts) {
...
contract('FlatsharesToken (transfers)', function (accounts) {
    const owner = accounts[0];
    const user1 = accounts[1];
...
contract('FlatsharesToken (token burning tests)', function (accounts) {
    ...
    beforeEach(async function () {
        token = await FlatsharesToken.new(owner);
    });
