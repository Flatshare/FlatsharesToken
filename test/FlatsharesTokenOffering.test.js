...
contract('FlatsharesTokenOffering constructor', function (accounts) {
    ...
    it('Constructor failure cases', async function () {
        const token = await FlatsharesToken.new(admin, { from: owner });
        // etherToFlatshares rate is 0
        await util.assertRevert(FlatsharesTokenOffering.new(
...
contract('Offering stage changes correctly', function (accounts) {
    ...
    it('Start and end offering correctly', async function () {
        const token = await FlatsharesToken.new(admin, { from: owner });
        const offering = await FlatsharesTokenOffering.new(
...
contract('Whitelist Crowdsale', function (accounts) {
    ...
    beforeEach(async function () {
        token = await FlatsharesToken.new(admin, { from: owner });
        offering = await FlatsharesTokenOffering.new(
            5000, beneficiary, util.toEther(1), token.address, { from: owner }
        );
