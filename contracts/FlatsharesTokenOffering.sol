...
import "./FlatsharesToken.sol";
import "zeppelin-solidity/contracts/lifecycle/Pausable.sol";

contract FlatsharesTokenOffering is Pausable {
     ...
     *  
     * @param shrToEtherRate Number of shrtokens per ether
     ...
     */
    function FlatsharesTokenOffering(
        uint256 shrToEtherRate, 
        address beneficiaryAddr, 
        uint256 baseContributionCapInWei,
        address tokenAddress
    ) public {
        require(shrToEtherRate > 0);
        require(beneficiaryAddr != address(0));
        require(tokenAddress != address(0));
        require(baseContributionCapInWei >= MINIMUM_CONTRIBUTION);

        token = FlatsharesToken(tokenAddress);
        rate = shrToEtherRate;
    ...
    function updateRate(uint256 shrToEtherRate) public onlyOwner atStage(Stages.Setup) {
        rate = shrToEtherRate;
    }
     ...
     *
     * @param to Participant address to send shrtokens to
     * @param tokens Amount of shrtokens to be sent to parcitipant 
     */
